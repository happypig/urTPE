...
check [facts_2_portals.md](facts_2_portals.md) for the latest analysis and updates
...
## 16. Session Findings (2026-08-24) — Wrong-Match Bugs + Per-Node Link Fix

Probing 6 projects for 建築執照/開工/使用執照 across both portals surfaced five bugs
(two were listed in the session summary; root causes traced and fixed same day).

### 16.1 Bug inventory

| # | Bug | Root cause | Status |
|---|---|---|---|
| 1 | 河堤段263-19 twur → view/1042 (新北市板橋區 project!) | `data/taipei_case_ids.json` curated fallback had wrong view_id (with bogus case_id "10421234") | **FIXED** (mapping → view/262) |
| 2 | 河堤段263-19 cache carried 板橋's national_milestones (事業計畫核定 112.12.29) | discovery wrote milestones parsed from the wrong view page | **FIXED** (cache + view.html refreshed from live view/262) |
| 3 | Per-node case mislink: 10204032 (核定 2016/07/05) attached to recno 772 (2019-08-01) instead of recno 1042 (2016-07-05) | positional assignment in `attach_links_to_projects` (`city_case_ids[0]` by track keyword) | **FIXED** (date-aligned anchoring, positional kept as fallback) |
| 4 | 金華段513-3: cases 10011041/10011042 attached to NO node | same positional logic (track keywords didn't hit 都市更新計畫 nodes) | **FIXED** (same date-aligned anchoring) |
| 5 | Latent wrong match in fetch script: `parcel in html` bare-substring check would match 奇岩段444等7筆 → view/264 (444等**17**筆) | `find_matching_view` in scripts/fetch_remaining_national_portal.py | **FIXED** (requires `<parcel>地號` + title section/parcel/count tuple match) |
| 6 | 金華段513-3: portal title 等**11**筆 vs Taipei 等**13**筆 → land-core join miss → no twur despite portal page existing (view/265, 實施者 江陵 confirmed) | land core includes parcel count; portals disagree on count | **OPEN — needs OpenSpec change** |
| 7 | 開工日期/使用執照 (third.ashx `Eng_Start_Date`/`Ulic_Date`) absent from pipeline & data model | third.ashx never integrated (facts §10.4 documented the endpoint only) | **OPEN — needs OpenSpec change** |

### 16.2 National portal 推動歷程 vocabulary (definitive)

Across 109 cached view pages + live checks of completed case view/136, the 推動歷程
table only ever carries: 事業計畫/權利變換計畫 申請·核定日期, 第一次變更…核定日期,
**使用核發日期** (completed cases only), 備註. The national portal **never** publishes
建築執照 or 開工日期 — "all three dates on both portals" is unsatisfiable by design;
the only cross-portal overlap is 使用執照 ↔ 使用核發日期.

### 16.3 Fixes applied (conformance repairs, no OpenSpec delta needed)

- `data/taipei_case_ids.json`: 河堤段263-19 → view_id 262, real case_ids.
- Cache repair (scripts/repair_cache_2026_08_24.py): 河堤段263-19 result.json now
  carries view/262 + its true milestones (事業計畫申請 102.05.31, 核定 105.07.06,
  第一次變更 108.08.01, 使用核發 110.10.25) + refreshed view.html; both 河堤段 and
  金華段 caches gained `case_milestones` (per-case 核定日期) for date anchoring.
- `urtpe/links.py`: `DiscoveryResult.case_milestones` retained in discovery;
  ...
  (exact, then ±1 day), falling back to the legacy positional heuristic when
  dates can't disambiguate (old caches). Implements the existing spec scenario
  "Per-stage city links land on the right node" — no spec change required.
- `scripts/fetch_remaining_national_portal.py`: strict `view_page_matches()`
  (parcel+地號 context + `parse_name_id` title tuple incl. count); candidates
  now carry land count.
- Tests: 2 regression tests added (date-aligned attach; positional fallback for
  old caches). Suite: 121 passed. Viewer regenerated (697 resolved / 12 unresolved).

### 16.4 Deferred to OpenSpec changes

1. **third.ashx integration** — new capability: fetch 開工/使用執照 per case, extend
   data model + viewer milestones (schema_version → 2 candidate). No spec exists yet.
2. **Parcel-count mismatch normalization** — land-core join should tolerate
   portal-vs-Taipei count drift (金華段 11 vs 13) without weakening uniqueness;
   touches land-identity-fallback / official-link-discovery semantics.
