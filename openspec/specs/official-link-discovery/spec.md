...
`search_taipei_cases_api` SHALL keep a searched case only when the case's own `case_name` contains the searched parcel — the anchor record's named first parcel (mono part; sub-parcel suffix tolerated in 之 ↔ - and full-width ↔ ASCII forms) — extending the §6.7 guard to all cross-family pollution. Cases whose name lacks the searched parcel (sibling R13 概要 cases, foreign same-section cases) SHALL NOT enter `city_case_ids`, and therefore SHALL NOT contribute milestones to the project's merged timeline.
